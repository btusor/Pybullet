...
amplitude = 0.3
freq = 0.2
Rd = np.array([[1, 0, 0], [0, -1, 0], [0, 0, -1]]) # Fixed Downward Orientation
...
print("Running Impedance Control with Feed-Forward Tracking...")
...
try:
    while True:
        ...
        # A. Target Position AND Velocity (Feed-Forward)
        target_p = center_pos + np.array([0, amplitude * np.sin(2 * np.pi * freq * t), 0])
        # Derivative of sine: A * 2 * pi * f * cos(...)
        target_v_lin = np.array([0, amplitude * 2 * np.pi * freq * np.cos(2 * np.pi * freq * t), 0])
        
        p.resetBasePositionAndOrientation(target_marker, target_p, [0,0,0,1])

        # B. Get Current State
        ee_state = p.getLinkState(robot_id, ee_index, computeLinkVelocity=1)
        curr_p, curr_q = np.array(ee_state[0]), ee_state[1]
        curr_v_lin = np.array(ee_state[6]) # Current linear velocity
        curr_v_ang = np.array(ee_state[7]) # Current angular velocity
        ...
        # C. Geometric Spring (Proportional Term)
        ...
        # D. Task-Space Damping (Velocity Tracking Term)
        # This acts as a damper between the EE and the MOVING target, 
        # reducing the lag significantly.
        Kv_task = 50.0 
        f_damping = Kv_task * (target_v_lin - curr_v_lin)
        m_damping = 5.0 * (np.zeros(3) - curr_v_ang) # Damping angular drift

        # E. Total Wrench & Jacobian
        ...
        tau_task = J.T @ Wrench
        tau_joint_damping = -B_joint * dq_active # Safety damping at joint level
